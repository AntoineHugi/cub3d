...
/*   Updated: 2025/07/18 09:31:46 by bokwuful         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	initialise_textures(t_game *game)
{
	...
	i = 0;
	while (i < MAX_SPRITE_FRAMES && game->s_texture_paths[i])
	{
		game->sprite_tex[i] = new_texture(game->mlx, game->s_texture_paths[i], game);
		i++;
	}
}
...
int	initialise_game(t_game *game)
{
	...
	if (game->frame.ptr == NULL)
		initialisation_error(game, "Error\nFrame image initialisation failed.");
	...
	if (game->frame.data == NULL)
		initialisation_error(game, "Error\nFrame data initialisation failed.");
